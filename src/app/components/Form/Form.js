...
export default function Form() {
  ...
  const handleSubmit = async (e) => {
    ...
    const extention = pendukung.name.split('.')[1];
    console.log(pendukung);

    const filePendukung = new FormData();
    const metadata = {
      name: `${tanggalSekarang}-${pelapor}-${telepon}.${extention}`,
      mimeType: pendukung.type,
      parents: ['1OlhdqjrcXnxq0IbuAAR6Jo9Nkfjmrvsj'],
    };
    filePendukung.append(
      'metadata',
      new Blob([JSON.stringify(metadata)], { type: 'application/json' })
    );
    filePendukung.append('pendukung', pendukung);
    filePendukung.append('token', session.accessToken);
      ...
      {session && (
        <div className='w-full p-4 sm:p-6 md:p-8 flex flex-col justify-center items-center bg-gradient-to-r from-cyan-500 to-blue-500 font-sans'>
          <div className='w-full sm:w-11/12 md:w-3/4 lg:w-1/2 xl:w-2/5 bg-white flex flex-col gap-4 rounded-lg shadow-xl p-6 sm:p-8 space-y-4'>
            {/* Form Header */}
            <div className='flex border-b border-gray-300 pb-4 sm:pb-6'>
              ...
              <button
                ...
                className={`w-[20%] text-center border border-transparent rounded-md text-white text-base font-medium bg-red-500`}>
                Logout
              </button>
            </div>
